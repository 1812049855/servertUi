...
public class ProgressBarView extends JDialog {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					ProgressBarView window = new ProgressBarView(null);
	...
	public ProgressBarView(Dialog owner) {
		super(owner, false);
		initialize();
	}
	public static void  string(String text){
		jLabel.setText("正在下载"+text+"文件");
	}


	public static void Show(Dialog owner, Thread thread) {
		ProgressBarView progressBarView = new ProgressBarView(owner);
		thread.start();
	...
	private void initialize() {
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				do {
					currentProgress = ftp.getDownloadWatcher().getPercent();
				...
				} while (!ftp.getDownloadWatcher().isFinished());
				try {
					Thread.sleep(2000);
