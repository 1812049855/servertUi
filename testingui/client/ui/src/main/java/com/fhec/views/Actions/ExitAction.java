...
public class ExitAction extends AbstractActionListener<MainWindow> {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (showConfirmDialog == 0) {
            Log.info("退出程序");
            exit();
           /* Thread th=new Thread(new ExitUpload());
            th.start();*/
        } else {
    ...
    private  void  exit(){
        ...
        //指令
        entityVo.setCommand(GlobalContext.getCommandConfig().getExit());
        //连接状态
        entityVo.setStatus(0);
        //选择模式
        entityVo.setMode("MES");
        //每一行数据 barcode
